...
def generate_anki_package(
    ...
    cloze_model = genanki.Model(
        constants.ANKI_MODEL_ID + 1,
        'VocabFlow Cloze Model',
        fields=[
            {'name': 'Sentence'}, {'name': 'Meaning'}, {'name': 'FullSentence'},
        ],
        templates=[{
            'name': 'Cloze Card',
            'qfmt': '<div class="example" style="font-style:normal;font-size:24px;">{{Sentence}}</div>'
                    '<div class="meaning" style="font-size:18px;margin-top:8px;">💡 点击查看答案</div>',
            'afmt': '<div class="example" style="font-style:normal;font-size:24px;">{{FullSentence}}</div>'
                    '<hr>'
                    '<div class="meaning">{{Meaning}}</div>',
        }], css=CSS
    )

    active_model = cloze_model if card_type == "cloze" else model
    ...
    with tempfile_mod.TemporaryDirectory() as tmp_dir:
        ...
        for idx, card in enumerate(cards_data):
            ...
            ct = card.get('ct', card_type)
            if ct == "cloze":
                # Cloze fields: Sentence (with blank) | Meaning | FullSentence
                fields = [phrase, meaning, raw_example]
                note_model = cloze_model
            else:
                fields = [
            ...
            if note_id:
                note = genanki.Note(model=note_model, fields=fields, guid=note_id)
            else:
                note = genanki.Note(model=note_model, fields=fields)
