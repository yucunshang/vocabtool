...
with tab_extract:
    ...
    if st.session_state.get('gen_words_data'):
        ...
        st.markdown("#### 🏷️ 词汇一览")
        st.caption("💡 直接点击单词方块可快速查询释义（结果就在下方显示）")

        if "word_block_lookup_last_query" not in st.session_state:
            st.session_state["word_block_lookup_last_query"] = ""
        if "word_block_lookup_last_result" not in st.session_state:
            st.session_state["word_block_lookup_last_result"] = None
        if "word_block_pills_last" not in st.session_state:
            st.session_state["word_block_pills_last"] = ""
            ...
            for j, (word, _) in enumerate(row):
                with cols[j]:
                    if st.button(word, key=f"wb_{i + j}", use_container_width=True):
                        st.session_state["_word_block_lookup_pending_word"] = word
                        st.rerun()

        pending_block_word = st.session_state.pop("_word_block_lookup_pending_word", "")
        if pending_block_word:
            with st.spinner(f"🔍 正在查询 {pending_block_word}..."):
                st.session_state["word_block_lookup_last_result"] = get_word_quick_definition(pending_block_word)
                st.session_state["word_block_lookup_last_query"] = pending_block_word

        block_lookup_result = st.session_state.get("word_block_lookup_last_result")
        if block_lookup_result and "error" not in block_lookup_result:
            raw_content = block_lookup_result.get("result", "")
            rank = block_lookup_result.get("rank", 99999)
            ...
            formatted_lines = []
            clickable_words = []
            current_query = st.session_state.get("word_block_lookup_last_query", "").lower().strip()
            ...
            for line in lines:
                ...
                if line.startswith("🌱"):
                    ...
                    formatted_lines.append(
                        f'<div style="color:#065f46;background:#ecfdf5;padding:6px 10px;border-radius:8px;margin:8px 0;line-height:1.7;">{safe}</div>'
                    )
                elif "|" in line and len(line) < 50:
                    safe = html.escape(line)
                    formatted_lines.append(
                        f'<div style="color:#1e3a8a;margin-bottom:6px;font-size:16px;line-height:1.7;">{safe}</div>'
                    )
                elif line.startswith("•"):
                    safe = html.escape(line)
                    formatted_lines.append(
                        f'<div style="color:#374151;margin-top:6px;font-size:16px;line-height:1.7;">{safe}</div>'
                    )
                else:
                    safe = html.escape(line)
                    formatted_lines.append(
                        f'<div style="color:#6b7280;margin-bottom:8px;font-size:16px;line-height:1.7;">{safe}</div>'
                    )

            display_html = "".join(formatted_lines)
            rank_badge = (
                f'<span style="display:inline-block;background:{rank_color};color:white;'
                f'padding:3px 10px;border-radius:5px;font-size:13px;font-weight:600;">'
                f'📊 {rank} · {rank_label}</span>'
            )
            st.markdown("#### 🔎 方块查词结果")
            st.markdown(
                f"""<div style="padding:4px 0;">{display_html}<div style="margin-top:10px;">{rank_badge}</div></div>""",
                unsafe_allow_html=True,
            )

            if clickable_words:
                picked_block = st.pills(
                    "继续查询（方块结果）",
                    options=clickable_words[:20],
                    key="word_block_lookup_pills",
                    label_visibility="collapsed",
                )
                if picked_block and picked_block != st.session_state["word_block_pills_last"]:
                    st.session_state["word_block_pills_last"] = picked_block
                    st.session_state["_word_block_lookup_pending_word"] = picked_block
                    st.rerun()
        elif block_lookup_result and "error" in block_lookup_result:
            st.error(f"❌ 方块查词失败：{block_lookup_result.get('error', '未知错误')}")
